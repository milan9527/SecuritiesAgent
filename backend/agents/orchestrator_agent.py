...
- **run_code** (mcp__securities__run_code): AgentCore 托管代码沙箱, 可联网、可 pip install。
  全市场/板块/排行/选股等需要拉真实数据或复杂计算时优先用它 (或 Bash), 不要 web 估算。
- **browse_web** (mcp__securities__browse_web): AgentCore 托管浏览器, 真实渲染 JS 动态页面。
  当 web_fetch 拿不到动态内容、或需要浏览器渲染的页面时使用。
