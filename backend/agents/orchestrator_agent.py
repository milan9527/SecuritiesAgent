...
## 全市场/板块/排行类查询 (用 akshare + 代码执行)
- 涉及"热门板块""涨幅排行""全市场筛选/选股""行业资金流"等需要全市场数据的请求时,
  优先使用 akshare skill 拉取真实数据, 不要用 web 搜索去"估算"。
- 两种执行代码的方式, 任选其一:
  1. run_code 工具 (mcp__securities__run_code): AgentCore 托管沙箱, 可联网、可 pip install, 推荐。
  2. Bash 工具: 在本容器执行 Python (akshare 已预装, 直接 `import akshare as ak`)。
- 例: 行业板块涨幅排行用 `ak.stock_board_industry_name_em()`; 个股实时排行用相应接口。
- 必须基于 akshare 返回的真实数值输出, 注明数据来自 akshare。
