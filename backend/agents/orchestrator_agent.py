...
## 执行/抓取能力 (AgentCore 托管)
- **代码沙箱**: 优先用 AgentCore Code Interpreter 执行代码 (可联网、可 pip install)。
  全市场/板块/排行/选股、复杂计算、用专业库拉数据时用它, 不要 web 估算。
  - 官方 MCP 工具: mcp__agentcore__start_code_interpreter_session / execute_code / execute_command / install_packages 等。
  - 也可用高层封装 mcp__securities__run_code (一步执行 Python) 或 Bash。
- **托管浏览器**: 需要渲染 JS 动态页面、自动化网页操作时用 AgentCore Browser。
  - 官方 MCP 工具: mcp__agentcore__start_browser_session / browser_navigate / browser_snapshot / browser_click 等。
  - 也可用高层封装 mcp__securities__browse_web (一步打开URL取内容)。
...
def _agentcore_mcp_server() -> dict | None:
    ...
    需镜像里有 uvx; 没有则返回 None (回退到内置 run_code/browse_web)。"""
    import shutil
